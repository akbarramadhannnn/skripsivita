...
import { getDetailAlternatif, editAlternatif } from '../../api';
import { Row, Col, Card, CardHeader, CardBody, CardFooter,FormText, FormGroup, Label, Input, Button, Alert } from 'reactstrap'

const EditAlternaitf = (props) => {
    const idALternatif = props.match.params.id
    ...
    useEffect(() => {
        getDetailAlternatif(idALternatif)
        .then(res => {
            if(res) {
                setStateKode(res.data.kode)
                setStateMerk(res.data.merk)
                setStateSensor(res.data.sensor)
                setStateResolusi(res.data.resolusi)
                setStateHarga(res.data.harga)
                setStateIso(res.data.iso)
                setStateFitur(res.data.fitur)
                setStateKeterangan(res.data.keterangan)
            }
        })
    }, [idALternatif])
    ...
    const simpanData = (e) => {
        ...
        if(stateKode === '' && stateMerk === '' && stateSensor === '' && stateResolusi === '' && stateHarga === '' && stateIso === '' && stateKeterangan === '') {
            ...
            const data = {
                ...
                merk: stateMerk,
                sensor: stateSensor,
                resolusi: stateResolusi,
                harga: stateHarga,
                iso: stateIso,
                fitur: stateFitur,
                keterangan: stateKeterangan
            }
            editAlternatif(idALternatif,data)
            .then(res => {
                if(res) {
                    setStateKode(res.data.kode)
                    setStateMerk(res.data.merk)
                    ...
                    setStateFitur(res.data.fitur)
                    setStateKeterangan(res.data.keterangan)
                    setMessage('data alternatif berhasil edit')
                    setTimeout(() => {
                        setMessage('')
                    }, 3000);
                }
